...
zx_status_t ProxyDevice::Rpc(pdev_req_t* req, uint32_t req_length, pdev_resp_t* resp,
    ...
    if (status != ZX_OK) {
        ...
        zxlogf(ERROR, "ProxyDevice::Rpc resp_size too short: %u\n", resp_size);
        status = ZX_ERR_INTERNAL;
        goto fail;
    } else if (handle_count != out_handle_count) {
        zxlogf(ERROR, "ProxyDevice::Rpc handle count %u expected %u\n", handle_count,
                out_handle_count);
        status = ZX_ERR_INTERNAL;
        goto fail;
    }
...
zx_status_t ProxyDevice::MapMmio(uint32_t index, uint32_t cache_policy, void** out_vaddr,
    ...
    auto status = Rpc(&req, sizeof(req), &resp, sizeof(resp), nullptr, 0, &vmo_handle, 1, nullptr);
    if (status != ZX_OK) {
        return status;
    }
    ...
    *out_size = resp.mmio.length;
    *out_handle = vmo_handle;
    if (out_paddr) {
        *out_paddr = resp.mmio.paddr;
    }
    *out_vaddr = (void *)(virt + resp.mmio.offset);
    return ZX_OK;
